...
fn run_smoke() -> Result<i32> {
    ...
    println!(
        "[smoke] partition + vCPU created OK (tsc_hz={})",
        cached_tsc_hz()
    );
...
fn run_blk_selftest() -> Result<i32> {
    ...
    let issue =
        |avail_slot: u16, d: u64, buf_gpa: u64, rtype: u32, sector: u64, num_sectors: u32| -> u8 {
            let hdr = buf_gpa;
        ...
        match args[i].as_str() {
            ...
            "--net-backend" => {
                ...
                let backend = NetBackendKind::from_str(k).ok_or_else(|| {
                    Error::msg(format!(
                        "--net-backend: unknown '{k}' (want loopback|nat|wintun)"
                    ))
                })?;
            ...
            "--virtio-9p-share" => {
                ...
                if p9_shares.iter().any(|s| s.tag == tag) {
                    return Err(Error::msg(format!(
                        "--virtio-9p-share: duplicate tag '{tag}'"
                    )));
                }
                if p9_shares.len() >= 8 {
...
fn run_pvh_run(args: &[String]) -> Result<i32> {
    ...
    let watchdog = if cfg.watchdog_secs > 0 {
        ...
        Some(std::thread::spawn(move || {
            for s in 1..=secs {
                ...
                let cpuid: u64 = counters
                    .iter()
                    .map(|c| c.cpuid.load(Ordering::Relaxed))
                    .sum();
    ...
    let console_input = if window_owns_input {
        None
    } else {
        input_devices
    };
    ...
    let io: u64 = vcpu_counters
        .iter()
        .map(|c| c.io.load(Ordering::Relaxed))
        .sum();
    let mmio: u64 = vcpu_counters
        .iter()
        .map(|c| c.mmio.load(Ordering::Relaxed))
        .sum();
    let cpuid: u64 = vcpu_counters
        .iter()
        .map(|c| c.cpuid.load(Ordering::Relaxed))
        .sum();
    let msr: u64 = vcpu_counters
        .iter()
        .map(|c| c.msr.load(Ordering::Relaxed))
        .sum();
    let halt: u64 = vcpu_counters
        .iter()
        .map(|c| c.halt.load(Ordering::Relaxed))
        .sum();
    let other: u64 = vcpu_counters
        .iter()
        .map(|c| c.other.load(Ordering::Relaxed))
        .sum();
...
fn write_snapshot(
    ...
    for (i, nic) in cfg.nics.iter().enumerate() {
        ...
        jw.u64(&format!("nic{i}_mac"), mac_u64)
            ...
            .u64(
                &format!("nic{i}_portfwd_count"),
                nic.port_forwards.len() as u64,
            );
    ...
    for (i, c) in caps.iter().enumerate() {
        ...
        w.write_section(
            ...
            &vs::encode_regs(i, vs::ARCH_REGS, &c.arch),
        )?;
        ...
        w.write_section(
            ...
            &vs::encode_regs(i, vs::TIMING_REGS, &c.timing),
        )?;
    }
...
fn run_restore(args: &[String]) -> Result<i32> {
            ...
            match args[i].as_str() {
                "--drive" => {
                    ...
                    let spec = args.get(i).ok_or_else(|| {
                        Error::msg("--restore --drive wants <path>[,readonly|ro]")
                    })?;
    ...
    let mut caps: Vec<vs::CapturedVcpuState> = (0..vcpu_count)
        .map(|_| vs::CapturedVcpuState::default())
        .collect();
    ...
    for (idx, bytes) in &sec_devices {
        let t = transports
            .get(*idx as usize)
            .ok_or_else(|| Error::msg(format!("snapshot: PciDevice index {idx} out of range")))?;
    ...
    println!(
        "[restore] stopped: reason={:?} uart_tx={}",
        reason,
        com1.tx_bytes()
    );
    Ok(match reason {
...
fn push_abs(tab: &mut Vec<InputEvent>, x: i32, y: i32, cw: u32, ch: u32, abs_x: u16, abs_y: u16) {
    ...
    let ax = if cw > 1 {
        (cx * max / (cw as i64 - 1)) as u32
    } else {
        0
    };
    let ay = if ch > 1 {
        (cy * max / (ch as i64 - 1)) as u32
    } else {
        0
    };
...
fn spawn_input_reader(
    ...
    std::thread::spawn(move || {
        ...
        loop {
            let mut read: u32 = 0;
            let ok =
                unsafe { ReadConsoleInputW(hin, recs.as_mut_ptr(), recs.len() as u32, &mut read) };
            if ok == 0 || read == 0 {
                return;
            }
            hvc0.clear();
...
fn append_mouse_events(
    ...
    use crate::virtio::input::{
        ABS_AXIS_MAX, ABS_X, ABS_Y, BTN_LEFT, BTN_MIDDLE, BTN_RIGHT, REL_HWHEEL, REL_WHEEL,
    };
    ...
    if unsafe { GetConsoleScreenBufferInfo(hout, &mut info) } != 0 {
        ...
        let ax = if cols > 1 {
            rx * ABS_AXIS_MAX as i32 / (cols - 1)
        } else {
            0
        };
        let ay = if rows > 1 {
            ry * ABS_AXIS_MAX as i32 / (rows - 1)
        } else {
            0
        };
    ...
    *prev_buttons = cur;
    ...
    if m.dwEventFlags & MOUSE_WHEELED != 0 {
        ...
        if delta != 0 {
            ...
            tab.push(InputEvent::rel(
                REL_WHEEL,
                if clicks != 0 { clicks } else { delta.signum() },
            ));
        }
    }
