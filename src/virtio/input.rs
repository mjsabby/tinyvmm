...
impl VirtioDevice for InputDevice {
    ...
    fn apply_device_state(&self, bytes: &[u8]) {
        ...
        self.acked_features
            .store(s.acked_features, Ordering::Relaxed);
...
mod tests {
    ...
    fn event_helpers() {
        ...
        assert_eq!(
            InputEvent::key(BTN_LEFT, false),
            InputEvent::new(EV_KEY, BTN_LEFT, 0)
        );
        assert_eq!(
            InputEvent::rel(REL_WHEEL, -1),
            InputEvent::new(EV_REL, REL_WHEEL, u32::MAX)
        );
    ...
    fn keyboard_advertises_keys_and_leds_only() {
        ...
        assert_eq!(
            payload(InputKind::Keyboard, CFG_EV_BITS, EV_LED as u8),
            vec![0x07]
        );
    ...
    fn tablet_advertises_abs_rel_and_buttons() {
        ...
        // ABS_X|ABS_Y.
        assert_eq!(
            payload(InputKind::Tablet, CFG_EV_BITS, EV_ABS as u8),
            vec![0x03]
        );
        // REL_HWHEEL (bit 6) + REL_WHEEL (bit 8).
        assert_eq!(
            payload(InputKind::Tablet, CFG_EV_BITS, EV_REL as u8),
            vec![0x40, 0x01]
        );
