...
// Per-vCPU context needed to compute APIC-ID-dependent CPUID leaves. Threaded
// through `ResolveCpuid` so that leaves 0x01 (legacy 8-bit APIC ID) and
// 0x0B / 0x1F (x2APIC extended-topology enumeration) report the correct
// vCPU-specific identity. With multi-vCPU SMP, returning the host's APIC ID
// would cause Linux to log `[Firmware Bug]: CPU N: APIC ID mismatch` because
// our MADT advertises `local_apic_id == acpi_processor_uid == vcpu_index`.
struct CpuidContext {
    std::uint32_t vcpu_index;   // [0, vcpu_count)
    std::uint32_t vcpu_count;   // [1, kMaxVcpus]
};
...
//   * Leaf 0x00: raise max-standard-leaf to >= 0x1F if needed (so the guest
//                can reach 0x0B / 0x1F for x2APIC topology).
//   * Leaf 0x01: force ECX[24]=1 (tsc-deadline, see SetHideTscDeadline) and
//                ECX[31]=1 (hypervisor). Rewrite EBX[31:24] to the per-vCPU
//                initial APIC ID, and EBX[23:16] to vcpu_count (max logical
//                processors per package, used by legacy topology fallback).
//   * Leaf 0x06: force EAX[2]=1 (ARAT: APIC timer always running).
//   * Leaf 0x0B: synthesized x2APIC topology. Subleaf 0 = SMT level (1 thread
//                per core); subleaf 1 = Core level (vcpu_count cores per
//                package); subleaf >= 2 = invalid terminator. EDX returns the
//                per-vCPU full 32-bit x2APIC ID.
//   * Leaf 0x1F: mirrors leaf 0x0B; modern Linux probes 0x1F first.
...
CpuidResult ResolveCpuid(std::uint32_t leaf,
                         ...
                         const CpuidContext& ctx) noexcept;
