...
`capture_device_state`/`apply_device_state`); the transport handles the rest of
the ordering (PCI config / BAR remap → common-cfg → MSI‑X table → queues →
device-state).

virtio-input is snapshot-safe: its queues + device-state (feature/status,
config select/subsel, LED state, and the **last absolute pointer position**) are
captured, and the pointer position also lives in restored guest RAM, so the
...
*tinyvmm began as a Rust rewrite of an earlier C++ implementation; that C++ tree
has since been removed and this Rust tree is now the canonical implementation.*
