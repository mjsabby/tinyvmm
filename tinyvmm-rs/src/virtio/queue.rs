...
impl Virtqueue {
    ...
    fn avail_idx(&self) -> u16 {
        self.load_acq16(self.avail_gpa + 2).unwrap_or(self.last_avail)
    }
    fn avail_flags(&self) -> u16 {
        self.load_acq16(self.avail_gpa).unwrap_or(0)
    }
    fn avail_ring(&self, slot: u16) -> u16 {
        self.load16(self.avail_gpa + 4 + 2 * slot as u64).unwrap_or(0)
    }
    fn used_event(&self) -> u16 {
        self.load_acq16(self.avail_gpa + 4 + 2 * self.size as u64).unwrap_or(0)
    }
    ...
    fn store_used_ring(&self, slot: u16, id: u32, len: u32) {
        ...
        let _ = self.mem.write_bytes(self.used_gpa + 4 + 8 * slot as u64, &b);
    }
