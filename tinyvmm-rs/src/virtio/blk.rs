...
impl BlockDevice {
    pub fn new(mem: Arc<GuestMemory>, backend: Arc<BlockFile>, queue_max: u32) -> Arc<Self> {
        ...
        put_le(&mut cfg[CFG_DISCARD_SECTOR_ALIGN..CFG_DISCARD_SECTOR_ALIGN + 4], 1);
        put_le(
    ...
    fn drain(&self) {
        ...
        loop {
            ...
            if rtype == BLK_T_DISCARD || rtype == BLK_T_WRITE_ZEROES {
                ...
                for k in 0..nranges {
                    let base = k * 16;
                    let r_sector = u64::from_le_bytes(range_buf[base..base + 8].try_into().unwrap());
                    let num_sectors =
        ...
        let (head, used_len) = unsafe {
            *(*r).status_ptr = if (*r).failed { BLK_S_IOERR } else { BLK_S_OK };
            ...
            let used_len = if (*r).failed {
                ...
                (if (*r).rtype == BLK_T_IN { (*r).total_done } else { 0 }).wrapping_add(1)
            };
            ((*r).head_idx, used_len)
        };
...
impl VirtioDevice for BlockDevice {
    ...
    fn capture_device_state(&self) -> Vec<u8> {
        ...
        self.driver_features.load(Ordering::Relaxed).to_le_bytes().to_vec()
    }
