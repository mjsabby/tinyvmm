...
impl GpuDevice {
    ...
    fn cmd_display_info(&self, st: &mut GpuState) {
        ...
        st.resp.resize(CTRL_HDR_LEN + MAX_SCANOUTS * DISPLAY_ONE_LEN, 0);
    ...
    fn cmd_transfer_2d(&self, st: &mut GpuState) -> u32 {
        ...
        let ok = if rx == 0 && ry == 0 && rw == res.width && rh == res.height {
            ...
            for row in 0..rh as u64 {
                ...
                if !read_backing(&self.mem, &res.backing, src, &mut res.host[dst..dst + row_bytes])
                {
                    ok = false;
                    break;
                }
...
mod tests {
    ...
    fn write_response_scatters_and_skips_readable() {
        ...
        let chain = PoppedChain {
            head_index: 0,
            bufs: vec![
                ChainBuf { ptr: a.as_mut_ptr(), len: a.len(), write: true },
                ChainBuf { ptr: r.as_mut_ptr(), len: r.len(), write: false },
                ChainBuf { ptr: b.as_mut_ptr(), len: b.len(), write: true },
            ],
        };
