...
impl Pic8259 {
    ...
    fn handle_chip(&self, inner: &mut Inner, is_slave: bool, cmd_port: bool, acc: &mut IoAccess) {
        if acc.is_write {
            ...
            if cmd_port {
                if v & ICW1_IS_INIT != 0 {
                    let chip = if is_slave { &mut inner.slave } else { &mut inner.master };
            ...
            let step = if is_slave { inner.slave.step } else { inner.master.step };
            match step {
                IcwStep::Icw2 => {
                    let chip = if is_slave { &mut inner.slave } else { &mut inner.master };
                ...
                IcwStep::Idle => {
                    let prev = if is_slave { inner.slave.mask } else { inner.master.mask };
                    if is_slave {
                        inner.slave.mask = v;
                    } else {
                        inner.master.mask = v;
        ...
        // Read side.
        let chip = if is_slave { &inner.slave } else { &inner.master };
    ...
    pub fn snapshot_apply(&self, bytes: &[u8]) {
        ...
        let mut o = 0;
        for ci in 0..2 {
            let chip = if ci == 0 { &mut inner.master } else { &mut inner.slave };
