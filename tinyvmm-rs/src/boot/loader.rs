...
pub fn load_pvh(
    ...
    if let Some(img) = initramfs {
        ...
        if gpa > ram_bytes || img.len() as u64 > ram_bytes - gpa {
            return Err(Error::msg("initramfs would extend past configured guest RAM"));
        }
    ...
    mm.push(mk(BOOT_ARTIFACT_END, ISA_HOLE_START - BOOT_ARTIFACT_END, E820_RAM));
    mm.push(mk(ISA_HOLE_START, ISA_HOLE_END - ISA_HOLE_START, E820_RESERVED));
