...
impl NatState {
    ...
    fn wrap_and_inject(&mut self, src_ip: [u8; 4], dst_ip: [u8; 4], proto: u8) {
        build_ipv4_into(&mut self.tx_scratch, src_ip, dst_ip, proto, &self.l3_scratch);
        build_eth_into(
            &mut self.l3_scratch,
            self.guest_mac,
            self.gw_mac,
    ...
    fn handle_arp(&mut self, eth: &EthView) {
        ...
        build_eth_into(&mut self.tx_scratch, arp.sha, self.gw_mac, ETHERTYPE_ARP, &reply);
    ...
    fn new_tcp_flow(&mut self, key: FlowKey) -> Option<()> {
        ...
        let now = self.now();
        let cfg = TcbConfig {
            local: Endpoint { ip: dip, port: dport },
            remote: Endpoint { ip: gip, port: gport },
            iss,
            initial_rto_ms: 1000,
        };
